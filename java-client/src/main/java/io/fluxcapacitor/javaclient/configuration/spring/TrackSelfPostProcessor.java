...
import io.fluxcapacitor.javaclient.tracking.handling.SelfHandler;
...
public class TrackSelfPostProcessor implements BeanDefinitionRegistryPostProcessor {
    ...
    public void postProcessBeanFactory(@NonNull ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        Arrays.stream(beanFactory.getBeanNamesForAnnotation(TrackSelf.class))
                ...
                .flatMap(type -> SelfHandler.asSelfHandler(type, false).stream())
                .forEach(selfHandler -> registry.registerBeanDefinition(
                         selfHandler.getType().getName()+ "$$SelfTracked",
                         genericBeanDefinition(SelfHandler.class, () -> selfHandler).getBeanDefinition()));
    }
